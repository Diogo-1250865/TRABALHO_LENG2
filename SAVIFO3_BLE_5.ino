...
void setup() {
  ...
  BLEDevice::setMTU(100);   // aumenta o limite de bytes para caber a string toda
...
// Envia os sensores EM 3 NOTIFICAÇÕES pequenas (cada uma < 20 bytes)
// para nunca serem cortadas pelo limite do BLE.
void enviarSensores() {
  ...
  // ---- Notificação 1: temperatura, humidade, distância ----
  String parte1 = "T:" + String(temperatura, 1) +
                  ",H:" + String(humidade, 1)   +
                  ",D:" + String((int)distancia);
  pSensorChar->setValue(parte1.c_str());
  pSensorChar->notify();
  delay(60);   // pequena pausa para a app processar cada uma

  // ---- Notificação 2: IR atual + IR máximo ----
  String parte2 = "IR:" + String(valorIR) +
                  ",IRMAX:" + String((int)irMax);
  pSensorChar->setValue(parte2.c_str());
  pSensorChar->notify();
  delay(60);

  // ---- Notificação 3: tempo de voo ----
  String parte3 = "TempVoo:" + String(tempoVoo);
  pSensorChar->setValue(parte3.c_str());
  pSensorChar->notify();

  Serial.print("-> App: "); Serial.print(parte1);
  Serial.print(" | "); Serial.print(parte2);
  Serial.print(" | "); Serial.println(parte3);
}
